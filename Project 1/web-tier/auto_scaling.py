...
AWS_ACCESS_KEY_ID = 'AKIAZIVS4KWR3D2BD2BH'
AWS_SECRET_ACCESS_KEY = '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'
REGION = 'us-east-1'
...
while True:
    ...
    if action == "SCALE_OUT":
        for item in list(terminate_requests.keys())[:new_scaling]:
            terminate_requests[item] = 0
        ...
        for i in range(new_scaling):
            if running_instance[i] not in terminated_instances:
